...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < ccn.toString().length; i += 1) {
    let cardNum = parseInt(ccn.toString()[i], 10);
...
function toNaryString(num, n) {
  function convertToTargetStm() {
    let index = num % n;
    let quotient = Math.floor(num / n);
    const convertedArr = [index];

    while (quotient >= n) {
      index = quotient % n;
      quotient = Math.floor(quotient / n);
      convertedArr.push(index);
    }
    convertedArr.push(quotient);
    const result = convertedArr.reverse().join('');
    return (result);
  }
  return convertToTargetStm();
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const pathArr = (input, sep = '/') => rotate(splitStrings(input, sep))
    .filter(allElementsEqual);
  const commonPath = (input, sep = '/') => {
    if (pathArr(input).length === 1) {
      return '/';
    }
    return pathArr(input).map(elAt(0)).join(sep);
  };

  return commonPath(pathes).length > 1 ? `${commonPath(pathes)}/` : commonPath(pathes);
}
